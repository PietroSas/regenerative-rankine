...
#Entrada de Dados
print("\n***************** Dados de Entrada *****************\n")
print("-> Pressões entre 10 e 19999 KPa")
simples = int(input("Digite 0 para Rankine Simples ou 1 para Regenerativo: "))
baixa_pressao = int(input("1) Valor de Baixa Pressão (KPa): "))
alta_pressao = int(input("2) Valor de Alta Pressão (KPa): "))
temp_saturacao = tabela.interpolar_agua_saturada(alta_pressao, 0)
print("Temperatura de Saturação", temp_saturacao)
temp_super_aquec = float(input("3) Valor da T de super aquecimento (ºC): " ))
...
for i in range(0,len(pressao_intermediaria)):
    ...
    # s5 da tabela (Posição 11, Entropia vapor saturado)
    T5 = temp_super_aquec
    s5 = tabela.interpolar_mestre(p5, T5, [11, 4], 0)

    #h5 da tabela (Posição 8, Entalpia vapor saturado)
    ...
    print ("Pressão Inter. de Rendimento Máximo = ",pressao_intermediaria[i])
    print ("h1 = ",h1)
    print ("h2 = ",h2)
    print ("h3 = ",h3)
    print ("h4 = ",h4)
...
print ("Pressão Intermediaria = ",pressao_int_max)
...
try:
    print ("Título Saída Turbina = ", x7_lista[indice])    
except Exception:
    print("\nATENÇÃO!")
    print ("Você está perdendo quantidades alarmantes de Energia!")
    print ("A saída da turbina está superaquecida")
